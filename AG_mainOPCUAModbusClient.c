#ifdef COMPILE
sudo gcc -std=c99 -v -I/home/pi/open62541/ -I/home/pi/open62541/plugins/ -I/usr/include/libxml2/ -lpthread -lxml2 -lmodbus -lcrypto -lssl open62541.c  \
AG_PopulateOPCUANodes.c AG_ExtractXMLElementNames.c AG_Misc.c AG_StartOPCUAClient.c SV_Modbus.c AG_mainOPCUAModbusClient.c -o myNewModbusClient >& error-msg
...
int *initialiseModbusConnection(modbus_t *mb, char* ipaddress);
int readModbusData(modbus_t *mb, uint16_t *);
...
int main(int argc, char *argv[])
{
	...
	if (argc != 4)
	{
		printf("Usage : ./myNewModbusClient <Sensor IP> <Sensor Port> <OPCUA Server IP> \n");
		exit (0);
	}
	else
	{
...
//config1->stateCallback = stateCallback;
	...
	//else
		...
		*/
		...
		printf("AG_mainOPCUAModbusClient.c : just about to call modbus routine \n");
		modbus_t *mb1;
		int err = 0;
		uint16_t tab_reg[32];
		err = initialiseModbusConnection(mb1, "192.168.1.170");	// modbus server refers to Beckhoff CX9020
		if (err == -1)
		{
			printf("Error connnecting to modbus master at 192.168.1.170 \n");
			exit (-1);
		}
		printf("AG_mainOPCUAModbusClient.c : modbus connection initialisation success \n");
		sleep(5);

		while (1)
		{
			printf("before readModbusData() function \n");
			sleep(5);
			if (readModbusData(mb1, tab_reg) != -1)
			{
				printf("=============================== \n");
				printf("after readModbusData() function \n");
				printf("=============================== \n");
				sleep(5);
			...
			//retval = StartOPCUAClient(uaClient, r2_airgard_data_Id);
				...
				// update the OPC UA Address space with modbus data
			...
			{
				printf("error reading modbus data \n");
				break;
			}
			//sleep(9);
		}
