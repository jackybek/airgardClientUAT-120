...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		*/
		...
		UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Just about to call UA_Client_connectUsername() at line 355");
                retval = UA_Client_connectUsername(uaClient, OPCUAServerIP, "admin", "defaultadminpassword24");
                if (retval != UA_STATUSCODE_GOOD)
                {
                        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "NA_mainOPCUAClient.c : Cannot login securely to OPCUAServer : %s", OPCUAServerIP);
                        return(EXIT_FAILURE);
                }
		UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "UA_Client_connect : success");
